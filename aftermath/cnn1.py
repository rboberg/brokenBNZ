...
class CNN1(NNBase):
    ...
    def _acc_grads(self, X, y, just_probs=False):
        ...
        if self.context > 0:
            ...
            for i in range(self.context):
                try:
                    X = hstack((X,
                        vstack((zeros((min(ns,i+1),wdim)), X[:-(i+1),:wdim])),
                        vstack((X[(i+1):,:wdim], zeros((min(ns,i+1),wdim))))
                        ))
                except:
                    pdb.set_trace()
    ...
    def string_name(self):
        ...
        return(
            (
                'model=cnn1_wdim={wdim}_hdim={hdim}_odim={odim}_alpha={alpha}' +
...
if __name__ == '__main__':
    ...
    nnmx = CNN1(wdim=wdim,hdim=hdim,drop_p=drop_p, context=context)

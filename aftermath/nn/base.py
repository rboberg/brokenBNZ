...
class NNBase(object):
    ...
    def train_check_loss(self, X, y, counter, devidx, bothloss):
        """
        Used by train_sgd to compute, report, return cost
        """
        if not devidx is None:
            try:
                cost = self.compute_display_loss(X[devidx], y[devidx])
            except TypeError:
                cost = self.compute_display_loss(
                    [X[idx] for idx in devidx],
                    [y[idx] for idx in devidx])

            if bothloss:
                cost = (cost, self.compute_display_loss(X, y))
                print "  [%d]: mean dev loss %g, train loss %g" % (counter, cost[0], cost[1])
            else:
                print "  [%d]: mean dev loss %g" % (counter, cost)
        else:
            cost = self.compute_display_loss(X, y)
            print "  [%d]: mean train loss %g" % (counter, cost)
        return((counter, cost))
    ...
    def train_sgd(self, X, y,
                  ...
                  annealevery=0,
                  devidx=None, bothloss=True, savepath=None,
                  rseed=None, verbose=True):
        ...
        if alphaiter == None: # default training schedule
            if annealevery != 0:
                alphatype = 'anneal' + str(annealevery)
                alphaiter = self.annealiter(self.alpha, annealevery)
            else:
                alphatype = "default"
        ...
        try:
            ...
            for idx, alpha in itertools.izip(idxiter, alphaiter):
                if counter % printevery == 0:
                    print "  Seen %d in %.02f s" % (mbcounter, time.time() - t0)
                    print alpha
                if counter % costevery == 0:
                    costs.append(self.train_check_loss(X,y,counter=mbcounter,devidx=devidx, bothloss=bothloss))

                if hasattr(idx, "__iter__") and len(idx) > 1: # if iterable
                    self.train_minibatch_sgd(
                        [X[i] for i in idx],
                        [y[i] for i in idx],
                        alpha)
                    mbcounter += len(idx)
            ...
            """
            print "SGD Interrupted: saw %d examples in %.02f seconds." % (mbcounter, time.time() - t0)
            return costs
        ...
        costs.append(
            self.train_check_loss(
                X, y, counter=mbcounter,
                devidx=devidx, bothloss=bothloss)
            )
        #print "  [%d]: mean loss %g" % (counter, cost)
        print "SGD complete: %d examples in %.02f seconds." % (mbcounter, time.time() - t0)

        if not savepath is None:
            # Save the network, iterator params, and costs
            fname = self.string_name()

            iterstring = "counter={0}_mb={1}_alphaiter={2}".format(
                mbcounter, int(ceil(1.*mbcounter/counter)), alphatype)

            if not devidx is None:
                if isinstance(devidx, slice): devidx = range(len(y))[devidx]
                iterstring += "_dev0={0}_devn={1}_devlen={2}".format(
                    devidx[0], devidx[-1], len(devidx))

            #if isinstance(alphaiter, itertools.repeat):
                # it is a repeat object, which cannot be pickled
            #    alphaiter = None

            cucumber = {'nn':self, 'costs':costs,
                'counter':mbcounter, 'minibatch':mbcounter, 'annealevery':annealevery}

            fname += '_' + iterstring + '.bin'

            with open(savepath + '/' + fname, 'wb') as fid:
    ...
    @staticmethod
    def randomiter(N, pickfrom, batch=1):
        """Iterator to generate random minibatches."""
        for i in xrange(N):
            if hasattr(pickfrom, '__iter__'):
                yield random.choice(pickfrom, size=batch, replace=False)
            else: yield random.randint(0, pickfrom, size=batch)
